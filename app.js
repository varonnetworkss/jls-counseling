...
let db = null;
function loadDB(){
  try{ const raw = localStorage.getItem(DB_KEY); if(raw){ db = JSON.parse(raw); migrateDB(); ensureSemesters(); saveDB(); return; } }
  catch(e){ console.warn('DB load fail', e); }
  db = seedDB(); ensureSemesters(); saveDB();
}
/* 구버전 db에 없던 필드 보강 */
function migrateDB(){
  if(!db.uploadBatches) db.uploadBatches = [];
  if(!db.semesters) db.semesters = [];
}
function saveDB(){ localStorage.setItem(DB_KEY, JSON.stringify(db)); }
function resetDB(){ db = seedDB(); ensureSemesters(); saveDB(); }
...
function confirmReset(){
  openConfirm('전체 데이터 초기화','업로드한 전체명단·상담이력과 신규/퇴원 기록이 모두 삭제되고 빈 상태로 돌아갑니다. 분원 계정은 유지됩니다. 되돌릴 수 없습니다.',()=>{
    resetDB(); closeModal(); toast('초기화 완료','ok');
